...
/**
 * 
 * Esta es la clase Vector que solamente se utiliza para llamar a los métodos que van a poner visualizaciones de vectores o arrays en pequeñas ventanas con distintos tipos de datos.
 * 
...
public class Animador {
	...
	/**
	 * Este método crea un objeto Ventana y un vector de enteros, posteriormente recorre el vector de distintas formas que se visualizan en la ventana.
	 */
	public void ejemploVectorInt() {
		...
		/*
		 * El siguiente código ordena el vector! Se utiliza el algoritmo de selección
		 * 
		 * El for externo recorre cada posición i del vector, mientras que el for interno recorre todo lo que queda del vector hacia la derecha, buscando el elemento mínimo entre i y n - 1. El elemento mínimo se intercambia con i.
		 * 
		 * No se revisa nunca entre 0 y i - 1, porque la parte izquierda del vector siempre se encuentra ordenada.
		 */
		...
		JOptionPane.showMessageDialog(ventana,
				"Cuando haga click en Ok la ventana se va a cerrar.\nArrastre esta ventana para observar mejor el resultado",
	...
	public void ejemploVectorPersona() {
		...
		String nombres[] = { "Juan", "Agustino", "Carolina", "Fulgencio", "Picaporte" };
	...
	/**
	 * Crea una matriz de int y la muestra en pantalla
	 */
	public void ejemploMatrizInt() {

		// Declaración e inicialización de una matriz de 3x3
		// El primer par de paréntesis indica las filas
		// El segundo par de paréntesis indica las columnas
		int m[][] = new int[3][3];

		// For para tener un índice para recorrer filas
		for (int i = 0; i < m.length; i++) {

			// For para tener un índice para recorrer columnas
			for (int j = 0; j < m[i].length; j++) {

				// Se asigna un valor aleatorio en cada
		...
		Ventana ventana = new Ventana(m);

		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j < m[i].length; j++) {
				ventana.resaltar(i, j, Color.ORANGE);
			}
		}

		for (int i = 0; i < m.length; i++) {
			for (int j = m[i].length - 1; j >= 0; j--) {
				ventana.resaltar(i, j, Color.GREEN);
			}
		}
		...
		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j <= i && j < m[i].length; j++) {
				ventana.resaltar(i, j, Color.GREEN);
				ventana.marcar(i, j, Color.LIGHT_GRAY);
			}
		}

		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j < m[i].length; j++) {
				ventana.desmarcar(i, j);
			}
		}
		...
		while (c > 0) {
			...
			int j = rand.nextInt(m[0].length);

			if (!b[i][j]) { // Equivalente a b[i][j] == false
				ventana.resaltar(i, j, Color.YELLOW);
				ventana.marcar(i, j, Color.LIGHT_GRAY);
				b[i][j] = true;
				c--;
			}
		...
		JOptionPane.showMessageDialog(ventana, "El codigo se ejecuto " + n + " veces");

		// ventana.dispose();
	}

	public void ejemploRecorridoEnZ() {
		...
		this.recorrerEnZ(0, 0, m.length, m[0].length, ventana, m);

	}

	private void recorrerEnZ(int i, int j, int n, int m, Ventana ventana, int matriz[][]) {

		if (n > 1 && m > 1) {

			this.recorrerEnZ(i + 0, j + 0, n / 2, m / 2, ventana, matriz);
			this.recorrerEnZ(i + 0, j + m / 2, n / 2, m / 2, ventana, matriz);
			this.recorrerEnZ(i + n / 2, j + 0, n / 2, m / 2, ventana, matriz);
			this.recorrerEnZ(i + n / 2, j + m / 2, n / 2, m / 2, ventana, matriz);

		} else {
			ventana.resaltar(i, j, Color.CYAN);
		}
